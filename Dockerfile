...
RUN $POETRY_HOME/bin/poetry cache clear PyPI --all && $POETRY_HOME/bin/poetry cache clear _default_cache --all && $POETRY_HOME/bin/poetry install --no-interaction --no-ansi $(test "$IS_CI" = "True" && echo "--no-dev")
